...
function DiscoverBooksScreen() {
  ...
  /*const isLoading = status === statusConstants.loading
  const isSuccess = status === statusConstants.success
  const isError = status === statusConstants.error*/

  function handleSearchSubmit(event) {
    event.preventDefault()
    setQueried(true)
    console.log(`setQuery: ${event.target.elements[0].value}`)
